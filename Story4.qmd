...
df <- read.csv(path, ) |> 
  clean_names() |> 
  mutate(
    tot_emp = as.numeric(str_remove(tot_emp, ",")),
    jobs_1000 = as.numeric(str_remove(jobs_1000, ",")),
    across(starts_with("a_"), ~as.numeric(str_remove(., ",")))
  ) |> 
  select(area_title, prim_state, occ_title, tot_emp, jobs_1000, loc_quotient, starts_with("a_"))

df |> head()
```

```{r}
#| label: boxplots but w/ non-api data

df |> 
  ggplot(aes(x = occ_title, y = a_median/1000)) +
...
```{r}
#| label: boxplots but w/ widths adjusted for state population

df |> 
  mutate(jobs_adj = round(jobs_1000*1000)) |> 
  drop_na(jobs_adj) |> 
  uncount(jobs_adj) |>
  ggplot(aes(x = occ_title, y = a_median/1000)) +
  geom_boxplot(varwidth = T) +
...
distribution |> 
  ggplot(aes(x = occ_title, y = values/1000)) +
  geom_boxplot(varwidth = T) +
  # Edit theme
  theme_minimal() +
  theme(panel.grid.major = element_blank(),
...
```{r}
#| label: simualated distribution boxplots, by state

distribution |> 
  ggplot(aes(y = prim_state, x = values/1000)) +
  geom_boxplot(varwidth = T) +
  # Edit theme
  theme_minimal() +
  theme(panel.grid.major = element_blank(),
